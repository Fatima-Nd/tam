...
export function EditCompany(props) {
    ...
    const[currency,setcurrency]= useState(props.location.state.currency)
    const[vat,setvat]= useState(props.location.state.vat)
    const[start_date,setstart_date]= useState(moment(props.location.state.start_date).format("DD/MM/YYYY"))
    const[end_date,setend_date] = useState(moment(props.location.state.end_date).format("DD/MM/YYYY"))
    const[vat_amount,setvat_amount] = useState(props.location.state.vat_amount)
    const[vat_date,setvat_date] = useState(moment(props.location.state.vat_date).format("DD/MM/YYYY"))
    const[mof,setmof] =  useState(props.location.state.mof)
    const[commercial_register,setcommercial_register] = useState(props.location.state.commercial_register)
    const[image,setimage]= useState(props.location.state.image)
    const [phone1,setphone1] = useState(props.location.state.phone1)
    const[phone2,setphone2] = useState(props.location.state.phone2)
    const[fax,setfax] = useState(props.location.state.fax)
    const[email,setemail] = useState(props.location.state.email)
    const[country,setcountry] = useState(props.location.state.country)
    const[city,setcity] = useState(props.location.state.city)
    const[address,setaddress] = useState(props.location.state.address)
    const[website,setwebsite] = useState(props.location.state.website)
    ...
    return (
         <Card className='center-elements' >
            ...
            <Form onSubmit={editComapny} noValidate>
                <div className="p-fluid p-formgrid p-grid">
                        <div className="p-field p-col-1"></div>
                        <div className="p-field p-col">
                            <strong className='titles'>{translate('Update Company')}</strong>
                        </div>
                </div>
                <div className="p-fluid p-formgrid p-grid">
                <div className="p-field p-col-1"></div>
                ...
                <div className="p-field p-col-1"></div>
                    ...
                    <div className="p-field p-col">
                        ...
                        <Calendar placeholder={start_date} value={start_date} onChange={(e) => setstart_date(e.value)} monthNavigator yearNavigator yearRange="2010:2030" required className={classNames({ 'p-invalid': submitted && !start_date })} />
                    ...
                    <div className="p-field p-col">
                        ...
                        <Calendar placeholder={end_date} value={end_date} onChange={(e) =>setend_date(e.value)} required className={classNames({ 'p-invalid': submitted && !end_date })} />
                ...
                <div className="p-field p-col-1"></div>
                    ...
                    <div className="p-field p-col">
                        ...
                        <Calendar placeholder={vat_date} value={vat_date}  disabled={vat ? false : true} onChange={(e) => setvat_date(e.value)} monthNavigator yearNavigator yearRange="2010:2030" required={vat? true : false} className={vat ? classNames({ 'p-invalid': submitted && !vat_date }) : ''} />
